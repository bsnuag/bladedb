...
func (list *SkipList) setUnsafe(key string, value IndexRec) *Element {
	var element *Element
...
func (list *SkipList) Set(key string, value IndexRec) *Element {
	list.mutex.Lock()
	defer list.mutex.Unlock()

	return list.setUnsafe(key, value)
}

//set in batch - acquire lock once
func (list *SkipList) SetBatch(kvpairs map[string]IndexRec) {
	list.mutex.Lock()
	defer list.mutex.Unlock()

	for key, value := range kvpairs {
		list.setUnsafe(key, value)
	}
}
