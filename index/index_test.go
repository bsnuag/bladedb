...
func TestIndexSet(t *testing.T) {
	checkEqualIndexRec := func(rec1 bladedb.IndexRec, rec2 bladedb.IndexRec) bool {
		return rec1.TS == rec2.TS && rec1.SSTFileSeqNum == rec2.SSTFileSeqNum && rec1.SSTRecOffset == rec2.SSTRecOffset
	}
	index := NewIndex()

	indexRecs := make(map[string]bladedb.IndexRec, 100)

	for i := 0; i < 100; i++ {
		key := fmt.Sprintf("key_%d", i)
		indexRec := bladedb.IndexRec{
			SSTRecOffset:  uint32(i),
			SSTFileSeqNum: uint32(i),
			TS:            time.Now().UnixNano(),
		}
		indexRecs[key] = indexRec
		index.SetSafe(key, indexRec)
	}
...
func TestIndex_All(t *testing.T) {
	...
	set := index.SetSafe(key, indexRec)
